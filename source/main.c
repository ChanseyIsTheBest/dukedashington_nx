 ...
 *
 *   1. config.txt, duke.log, check the game files, chdir into assets/
...
int dd_exit_requested(void) { return g_exit_requested; }
int dd_exit_code(void)      { return g_exit_code; }
...
{
    LOGI("exit (code %d) after %.1f s", code, dd_uptime());
    log_flush();
    svcExitProcess();
}

void dd_park_forever(void)
{
...
static int error_screen(const char *msg)
{
    ...
    printf("Duke Dashington Remastered for Nintendo Switch could not start.\n\n");
    printf("%s\n\n", msg);
    if (paths_log()[0])
