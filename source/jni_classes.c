...
static const JClassDef class_MainActivity = { "com/adventureislands/dukedashingtonremastered/MainActivity", "org/haxe/lime/GameActivity", NULL, NULL, NULL };
...
{
    if (!g_package)
        g_package = jni_permanent_string(DD_PACKAGE);
    return g_package;
}
...
{
    (void)env; (void)self; (void)m;
    prefs_set("__dd_unityads_consent", args && args[0].z ? "1" : "0");
    return v_none();
}

IMPL(unity_getConsent)
{
    char *v = prefs_get("__dd_unityads_consent");
    int r = v && v[0] == '1';
    (void)env; (void)self; (void)args; (void)m;
    free(v);
    return v_bool(r);
