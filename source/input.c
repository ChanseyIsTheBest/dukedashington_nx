...
void input_init(void)
{
    ...
    LOGI("input: layout %s (%s jumps), dpad as %s, deadzone %d, touch %s%s",
         g_cfg.button_layout == DD_LAYOUT_LABEL ? "label" : "position",
         g_cfg.button_layout == DD_LAYOUT_LABEL ? "A" : "B",
         g_cfg.dpad == DD_DPAD_HAT ? "hat" : (g_cfg.dpad == DD_DPAD_STICK ? "stick" : "hat+stick"),
...
{
    const int label = g_cfg.button_layout == DD_LAYOUT_LABEL;
    ...
    set_hat(g_cfg.dpad == DD_DPAD_STICK ? 0 : hat);
}
...
static void update_combo(u64 held)
{
    ...
    if (g_combo_fired) {
        if (t - g_combo_fired_at > COMBO_FORCE_MS && !dd_exit_requested()) {
...
/* --------------------------------------------- pad as game controls -- */
/* Unlike the Total Party Kill build, this game is control-driven: its own
 * behaviours listen on named controls rather than reading raw touch. Read out
 * of libApplicationMain.so:
 *
 *   Design_230_230_DukeControls        Up, Left, Right, Restart
 *   Design_899_899_TitleKeyboardControls          Left, Right, Enter, Space, Esc
 *   Design_898_898_LevelSelectMenuKeyboard01      ditto, plus Up/Down
 *   Design_897_897_DungeonSelectKeyboardControls  ditto
 *
 * and com.stencyl.Input in this build carries the full control machinery
 * (_controlMap, loadInputConfig, mapKey/addKeyToControl, and the joystick
 * equivalents) that the Total Party Kill build did not have.
 *
 * Those control names are keyboard-shaped -- Enter, Space, Esc -- and the
 * same Stencyl project ships a keyboard-driven desktop release, so the port
 * drives the game by synthesizing the keys those controls bind to. That is
 * input_mode = keys.
 *
 * It also keeps the Total Party Kill approach as a fallback: the game does
 * draw an on-screen control bar (Left/Right/Up Button01 and a Pause Button
 * are in its actor list), and pressing those is input_mode = touch. Both run
 * by default, because a control that means nothing in the current scene is
 * simply ignored.
 */

/* The key each control is bound to. The pause button is on-screen only --
 * the game has no Pause control, just an actor you tap. */
typedef struct {
    SDL_Scancode scancode;
    SDL_Keycode  sym;
} ControlKey;

static const ControlKey CTL_KEY[CTL_COUNT] = {
    ...
    { SDL_SCANCODE_SPACE,   SDLK_SPACE   },   /* CTL_SPACE   */
    { SDL_SCANCODE_ESCAPE,  SDLK_ESCAPE  },   /* CTL_BACK    */
    { SDL_SCANCODE_R,       SDLK_r       },   /* CTL_RESTART */
    { SDL_SCANCODE_UNKNOWN, SDLK_UNKNOWN },   /* CTL_PAUSE   */
};

_Static_assert(sizeof(CTL_KEY) / sizeof(CTL_KEY[0]) == CTL_COUNT,
               "CTL_KEY must have one entry per control");

static int g_key_down[CTL_COUNT];
...
/* The control slots only. The cursor owns PTR_SLOT and presses it from
 * update_pointer, which runs first -- releasing everything here would lift the
 * cursor's finger again on the same poll. */
static void synth_release_controls(void)
{
    int c;
    for (c = CTL_NONE + 1; c < CTL_COUNT; c++)
        if (g_synth[c].down)
            synth_touch(c, 0, g_synth[c].x, g_synth[c].y);
}
...
/* Fold every input onto the controls it is mapped to: pressed[c] becomes 1 if
 * any input driving control c is being held. One input may drive several. */
static void fold_controls(u64 held, int *pressed)
{
    ...
    memset(pressed, 0, sizeof(int) * CTL_COUNT);
    ...
    for (i = 0; i < IN_COUNT; i++) {
        ...
        for (ctl = CTL_NONE + 1; ctl < CTL_COUNT; ctl++)
            if (g_cfg.map[i] & (1u << ctl))
                pressed[ctl] = 1;
    }
}

/* Send one key edge. Queued like every other synthesized event, so SDL's
 * event watchers do not see it; Lime reads these straight off the queue. */
static void key_event(int ctl, int down)
{
    SDL_Window *w = sdlb_window();
    SDL_Event e;

    /* Lime routes key events by window id; with no window it would drop them,
     * so do not queue one at all. Callers keep g_key_down in step. */
    if (!w)
        return;
    memset(&e, 0, sizeof(e));
    e.type = down ? SDL_KEYDOWN : SDL_KEYUP;
    ...
    e.key.state = down ? SDL_PRESSED : SDL_RELEASED;
    e.key.repeat = 0;
    e.key.keysym.scancode = CTL_KEY[ctl].scancode;
    e.key.keysym.sym = CTL_KEY[ctl].sym;
    push(&e);
    if (down)
        LOG_ONCE("input: pad drives the game's controls by key (first: %s)",
...
/* Press each control that any of its inputs is holding.
 *
 * A control goes in as a key if it has one, and as a press on the game's
 * on-screen button if it does not -- never as both. The key bindings are the
 * game's own (read out of the Stencyl config compiled into the binary); the
 * on-screen positions are estimates. Sending both would put a finger somewhere
 * on every press, and the level-select screens are covered in buttons, so a
 * stray one would tap a level while the player was only moving the selection.
 */
static void update_controls(u64 held)
{
    const int mode = g_cfg.input_mode;
    const int have_window = sdlb_window() != NULL;
    int pressed[CTL_COUNT];
    int ctl;

    fold_controls(held, pressed);

    for (ctl = CTL_NONE + 1; ctl < CTL_COUNT; ctl++) {
        const int has_key   = CTL_KEY[ctl].scancode != SDL_SCANCODE_UNKNOWN;
        const int want_key  = has_key && mode != DD_IN_TOUCH && have_window;
        const int want_touch = g_cfg.touch_buttons &&
                               g_cfg.touch_pos[ctl][0] >= 0.0f &&
                               (mode == DD_IN_TOUCH ||
                                (mode == DD_IN_BOTH && !has_key));

        if (want_key) {
            if (pressed[ctl] != g_key_down[ctl]) {
                g_key_down[ctl] = pressed[ctl];
                key_event(ctl, pressed[ctl]);
            }
        } else if (g_key_down[ctl]) {
            g_key_down[ctl] = 0;
            key_event(ctl, 0);
        }

        if (want_touch)
            synth_touch(ctl, pressed[ctl],
                        g_cfg.touch_pos[ctl][0], g_cfg.touch_pos[ctl][1]);
        else if (g_synth[ctl].down)
            synth_touch(ctl, 0, g_synth[ctl].x, g_synth[ctl].y);
    }
}
...
void input_pump(void)
{
    ...
    /* The cursor is modal: while it is up the stick drives it, so the control
     * mapping goes silent rather than fighting over the same stick. */
    if (!nxp_visible()) {
        update_controls(held);
    } else {
        synth_release_controls();
        keys_release_all();
    }
    if (g_cfg.touch)
        update_touch();
