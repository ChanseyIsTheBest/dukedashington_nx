#---------------------------------------------------------------------------------
# Duke Dashington Remastered -- Nintendo Switch homebrew wrapper port
#
...
TARGET      := dukedashington
BUILD       := build
SOURCES     := source
INCLUDES    := source

APP_TITLE   := Duke Dashington Remastered
...
#   make check GAME=sd/switch/dukedashington
check:
	@test -n "$(GAME)" || (echo "usage: make check GAME=path/to/switch/dukedashington"; exit 1)
	python3 tools/check_imports.py $(GAME)
	python3 tools/check_dynapi.py $(GAME)/liblime.so
