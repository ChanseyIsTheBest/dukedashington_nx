...
static int layout_ok(const char *dir)
{
    ...
    snprintf(g_log, sizeof(g_log), "%s/duke.log", g_root);
...
{
    static const char *const sub_names[] = { "dukedashington", "dukedashington_nx", "duke" };
    static const char *const known[] = {
        "sdmc:/switch/dukedashington", "sdmc:/switch/dukedashington_nx",
        "sdmc:/switch/duke", "sdmc:/dukedashington", "sdmc:/switch",
    };
    char dir[P], sub[Q];
    size_t i;
    DIR *d;
    ...
    snprintf(err, errlen,
             ...
             "Put these next to dukedashington.nro, in any folder:\n"
...
int paths_init(char *err, size_t errlen)
{
    ...
    if (!exists(probe, 0))
        LOGE("paths: %s is missing -- if this is not Duke Dashington Remastered, expect trouble", probe);
    return 1;
}
...
static PrefixMap prefixes(size_t i)
{
    static const char *const from[] = {
        "/storage/emulated/0/Android/data/" DD_PACKAGE "/files",
        "/sdcard/Android/data/" DD_PACKAGE "/files",
        "/data/data/" DD_PACKAGE "/files",
        "/data/user/0/" DD_PACKAGE "/files",
        "/data/data/" DD_PACKAGE,
        "/data/user/0/" DD_PACKAGE,
        "/storage/emulated/0",
        "/sdcard",
        "/android_asset",
    };
