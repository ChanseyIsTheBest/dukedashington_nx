...
static void *dd_SDL_AndroidGetJNIEnv(void) { return (void *)jni_get_env(); }
static void *dd_SDL_AndroidGetActivity(void) { return (void *)jni_ref(jni_activity()); }
static const char *dd_SDL_AndroidGetInternalStoragePath(void) { return paths_save_nodev(); }
static int dd_SDL_AndroidGetExternalStorageState(void) { return 3; }  /* read | write */
static const char *dd_SDL_AndroidGetExternalStoragePath(void) { return paths_save_nodev(); }
static SDL_bool dd_SDL_IsAndroidTV(void) { return SDL_FALSE; }
static SDL_bool dd_SDL_IsChromebook(void) { return SDL_FALSE; }
static SDL_bool dd_SDL_IsDeXMode(void) { return SDL_FALSE; }
static void dd_SDL_AndroidBackButton(void) {}
static int dd_SDL_GetAndroidSDKVersion(void) { return 29; }

/* ----------------------------------------------------------- helpers ---- */

static void output_size(int *w, int *h)
{
    if (g_cfg.resolution == DD_RES_1080 ||
        (g_cfg.resolution == DD_RES_AUTO && appletGetOperationMode() == AppletOperationMode_Console)) {
        *w = 1920;
        *h = 1080;
    } else {
        *w = 1280;
...
{
    (void)ud;
    if (prio < SDL_LOG_PRIORITY_INFO && log_get_level() < DD_LOG_DEBUG)
        return;
...
static SDL_Window *ovr_SDL_CreateWindow(const char *title, int x, int y, int w, int h, Uint32 flags)
{
    ...
    output_size(&ow, &oh);
    if (g_cfg.resolution == DD_RES_AUTO)
...
{
    if (!resizable && g_cfg.resolution == DD_RES_AUTO) {
...
#define SDLP_A(i, name) g_procs[i] = (void *)&dd_##name; g_names[i] = #name;
...
int32_t dd_SDL_DYNAPI_entry(uint32_t apiver, void *table, uint32_t tablesize)
{
